    ...
    <div className="fixed inset-0 z-[200] bg-gray-950 text-white overflow-auto">
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <div className="bg-red-900/50 border-2 border-red-500 rounded-2xl p-6 text-center">
          <p className="text-lg font-semibold text-red-300 mb-1">LIFE-THREATENING EMERGENCY</p>
          <p className="text-6xl font-black tracking-widest text-white">999</p>
          <p className="text-sm text-red-300 mt-2">
            <Phone className="w-4 h-4 inline mr-1" />
            Call <strong>999</strong> for Police, Fire, Ambulance | <strong>101</strong> for non-emergency
          </p>
        </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {alert.area && <span className="text-sm text-gray-400"><MapPin className="w-3 h-3 inline" /> {alert.area}</span>}
                    </div>
                    <h3 className="text-xl font-bold text-white">{alert.title}</h3>
                    <p className="text-sm text-gray-300 mt-1 leading-relaxed">{alert.description}</p>
        ...
        {alerts.length === 0 && !loading && (
          <div className="bg-green-900/30 border border-green-700 rounded-xl p-6 text-center">
            ...
            <p className="text-lg font-bold text-green-300">No Active Alerts</p>
            <p className="text-sm text-green-400/70">All clear — no emergency warnings at this time</p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="bg-gray-900 border border-gray-700 rounded-xl p-5">
            ...
            {highRiskPredictions.length > 0 ? (
                ...
                {highRiskPredictions.slice(0, 4).map((p, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center justify-between">
                      <span className="text-sm text-gray-300">{p.area}</span>
              ...
              <p className="text-sm text-gray-500">No high-risk flood predictions</p>
            )}
          </div>
          ...
          <div className="bg-gray-900 border border-gray-700 rounded-xl p-5">
            ...
            {shelters.length > 0 ? (
                ...
                {shelters.slice(0, 4).map(s => (
                  <div key={s.id} className="flex items-start justify-between gap-2">
                    ...
                    <div className="text-right flex-shrink-0">
                      <p className="text-xs text-gray-300">{s.current_occupancy}/{s.capacity}</p>
                      {s.phone && (
              ...
              <p className="text-sm text-gray-500">No shelters in database</p>
            )}
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              ...
              <a
                ...
                className="flex items-center gap-2 bg-gray-800 hover:bg-gray-700 rounded-lg p-3 text-sm text-gray-300 hover:text-white transition"
              >
        ...
        <div className="text-center text-xs text-gray-600 py-4 border-t border-gray-800">
          <p>
            ...
            Data from AEGIS, SEPA, Met Office, OpenWeather
          </p>
          <p className="mt-1">
            AEGIS Emergency Response System — All data from real-time sources
          </p>
        </div>
      </div>
    </div>
