    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-bold text-gray-800 dark:text-gray-200">Architecture Status Board</h2>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="font-semibold text-gray-700 dark:text-gray-300">AI Engine</span>
          </div>
          <StatusBadge ok={health.ai_engine.ok} />
          <p className="text-xs text-gray-400 mt-2">
            {health.ai_engine.ok ? `Latency: ${health.ai_engine.latency_ms}ms` : health.ai_engine.url}
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="font-semibold text-gray-700 dark:text-gray-300">Job Scheduler</span>
          </div>
          {(() => {
            // Determine scheduler health: fallback active with jobs succeeding = healthy
            const fallbackWorking = health.n8n.fallback_active && health.recent_jobs.length > 0
            const n8nConnected = health.n8n.status === 'connected' && health.n8n.healthy
            const isOperational = n8nConnected || fallbackWorking
            const notConfigured = health.n8n.status === 'not_configured'
            const label = n8nConnected
              ? 'n8n Connected'
              : notConfigured
                ? (fallbackWorking ? 'Fallback Active' : 'Not Configured')
                : fallbackWorking
                  ? 'Fallback Active'
                  : health.n8n.status === 'unreachable'
                    ? 'Unreachable'
                    : 'Starting...'
            return <StatusBadge ok={isOperational || (notConfigured && fallbackWorking)} label={label} />
          })()}
          <div className="text-xs text-gray-400 mt-2 space-y-0.5">
            ...
            {health.n8n.status === 'not_configured' && (
              <p className="flex items-center gap-1">
                <Zap className="w-3 h-3" />
                Internal cron scheduler running {health.recent_jobs.length} job types
              </p>
            )}
            {health.n8n.status === 'unreachable' && (
              <>
                <p className="text-amber-500">n8n unreachable ({health.n8n.consecutive_failures} failures)</p>
                {health.n8n.fallback_active && (
                  <p className="flex items-center gap-1">
                    <Zap className="w-3 h-3" />
                    Fallback scheduler active
                  </p>
                )}
              </>
            )}
            {health.n8n.last_check && (
              <p>Last check: {new Date(health.n8n.last_check).toLocaleTimeString()}</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="font-semibold text-gray-700 dark:text-gray-300">Errors (1h)</span>
          </div>
          <StatusBadge ok={totalErrors === 0} label={totalErrors === 0 ? 'Clean' : `${totalErrors} errors`} />
          <div className="text-xs text-gray-400 mt-2 space-y-0.5">
            <p>Frontend: {health.recent_errors.frontend}</p>
            <p>Backend: {health.recent_errors.system}</p>
            <p>External API: {health.recent_errors.external}</p>
          </div>
        </div>
      </div>
      ...
      {circuitBreakerEntries.length > 0 && (
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="font-semibold text-gray-700 dark:text-gray-300">External API Circuit Breakers</h3>
          </div>
      ...
      {health.recent_jobs.length > 0 && (
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="font-semibold text-gray-700 dark:text-gray-300">Recent Background Jobs</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-xs text-gray-500 border-b border-gray-200 dark:border-gray-600">
      ...
      {health.workflow_definitions && health.workflow_definitions.length > 0 && (
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="font-semibold text-gray-700 dark:text-gray-300">Registered Workflow Definitions</h3>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {health.workflow_definitions.map((wf, i) => (
              <div key={i} className="p-3 rounded-lg border border-gray-200 bg-gray-50 dark:border-gray-600 dark:bg-gray-700/50">
                ...
                <p className="text-xs text-gray-400">{wf.nodeCount} nodes</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="font-semibold text-gray-700 dark:text-gray-300">Architecture Data Flow</h3>
        </div>
        <div className="flex flex-wrap items-center justify-center gap-2 text-xs">
          ...
          <div className="flex flex-col items-center gap-1 p-2 rounded-lg bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-800 min-w-[80px]">
            ...
            <span className="text-green-700 dark:text-green-400 font-semibold">Backend</span>
          ...
          <div className={`flex flex-col items-center gap-1 p-2 rounded-lg border min-w-[80px] ${
            ...
            <span className="text-purple-700 dark:text-purple-400 font-semibold">AI Engine</span>
            <span className="text-purple-500 text-[10px]">FastAPI {health.ai_engine.ok ? `${health.ai_engine.latency_ms}ms` : 'Down'}</span>
          </div>
          <ArrowRight className="w-4 h-4 text-gray-400 flex-shrink-0" />
          ...
          <div className="flex flex-col items-center gap-1 p-2 rounded-lg bg-cyan-50 dark:bg-cyan-950/20 border border-cyan-200 dark:border-cyan-800 min-w-[80px]">
            ...
            <span className="text-cyan-700 dark:text-cyan-400 font-semibold">Frontend</span>
